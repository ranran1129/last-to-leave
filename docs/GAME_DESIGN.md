...
- **Sees**：目的の明示「客席・コンコース・搬入口の3か所へ流す」。入力1〜8のメーター（6だけ振れ、「音あり」と表示）。流す先＝客席スピーカー／コンコース／楽屋モニター（撤収済み）／搬入口 下手側 本線（断線）／下手側 予備／上手側 本線／上手側 予備。
...
- **Hints**：①卓のメーターが音の来ている入力を教えている。行き先はスタッフの書き置き。②撤収ボードは楽屋前の廊下。客席とコンコースは通常どおり。③下手側の本線が断線＝上手側の予備へ。
...
- **Sees**：入力ケーブルが届いていない分電盤、8系統のブレーカー、付箋「発電機 燃料わずか！ 同時3系統まで」、盤に貼られた注意書き「機材を降ろしたあとの回路（線の先に何もつながっていない状態）に送電しないこと。保護装置が働き、盤全体が落ちます。──いま館に残っている機材の系統だけ入れてください」。
- **Knows**：積込チェック表（☑＝積み込み済み＝もう館内にない／☐＝まだ館内に残っている。テープ色で記載：青テープ照明×6・黄テープ照明×2・赤テープ音響アンプ・青テープ音響アンプ・保温庫＝☑、赤テープ照明×6＝☐「明朝吊り下ろし後」。脚注「FOHと搬入口シャッターは会場の常設設備なのでこの表には載らない」）。ステージ前ケースの色＝上手/下手/中央。
- **Inference**：①まず電源ドラムで入力をつなぐ。②注意書きから「送電してよいのは機材が残っている系統だけ」と分かる（＝“なぜ残っているものを選ぶのか”の答え）。③色→上手/下手/中央に変換して積込済を消すと、残る機材は照明リグ（上手）だけ。④表に載らない常設設備（FOH・搬入口シャッター）を足してちょうど3系統＝付箋の上限と一致。
- **Feedback**：4つ目を入れると全落ち。空配線を選んだ場合は「〈○○〉は機材が運び出されたあとの空の回路だった」と、落ちた理由を名指しで返す。
- **World change**：分電盤の入力ランプ点灯、搬入口シャッターが上がる、照明リグが応答。
- **Hints**：①盤の注意書き＝残っている機材の系統だけ入れる。②何が運び出されたかは積込チェック表。色の意味はステージ前のケース。③赤＝上手・青＝下手・黄＝中央。残るのは照明リグ（上手）＋常設のFOH・搬入口シャッターの3つ。
