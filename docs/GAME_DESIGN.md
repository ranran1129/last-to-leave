...
- **物販の商品名**：実際の「濱岸ひより考案グッズ」8点（名称・税込価格ともユーザー提供の公式グッズ一覧に準拠）。
  1 好きなものかき集めビッグTシャツ ¥4,300／2 ばいころまる〜 Tシャツ ¥4,500／3 最近の濱岸ひよりステッカー（4枚1セット）¥1,200／4 いつも見えるところにひよたん貼っといてねッカー ¥900／5 ひよるのっぷ（ヘアクリップ）¥1,500／6 楽しさ二割増餃子皿 ¥4,000／7・8 ボンフィンキーホルダー 濱岸ひよりVer. Type-A / Type-B 各¥2,500
  → 商品ボードは 2列×4段。Type-B だけ完売マグネットが無く、付箋（一時完売→追加入荷）と食い違うので除外する必要がある。

**すべて創作**
- ブロック記号の具体的な割り付け、扉番号、出口番号、バックヤード・搬入口の配置。
...
- **他の客がいない理由**：案内⑥（最終）は 23:02 に読み上げられ、D5・E5・F5 の客もそれで退場している。
  抜けたのは**そのあとの目視確認だけ**。プレイヤーは案内を聞きながら立ち上がれず、そのまま座っていた（イントロと自席の描写）。
...
- **Sees**：商品一覧ボード（「濱岸ひより考案グッズ」2列×4段・番号と商品名と税込価格・完売マグネット7枚）、会計台の付箋群（商品名＋完売時刻、順不同、紛らわしい1枚）、扉のメモ「開け方はいつもどおり“売り切れた順”」、↑↓←→ だけの方向錠。
- **Inference**：付箋を時刻順に並べる → ボード上の位置へ置き換える → 隣り合うマスへの移動方向の列に変換。Type-B の付箋は「一時完売→追加入荷」でマグネットが無いので除外。
- **Answer**：7→5→6→4→3→1→2 ＝ ↑ → ↑ ← ↑ →
...
- **表記の方針**：卓の表示はすべて日本語。英略語（HOUSE / BS-SL など）は画面に出さず、内部IDとしてのみ持つ。
  また「やること」を地の文で説明しない。目的は現場にありそうな物（卓に貼りっぱなしの養生テープの手書き「閉館時 放送 → 客席・コンコース・搬入口」）で示す。
- **Sees**：入力1〜8のメーターと信号ランプ（6だけ振れて点灯）。流す先＝客席スピーカー／コンコース／楽屋モニター（撤収済み）／搬入口 下手側 本線（断線）／下手側 予備／上手側 本線／上手側 予備。
- **Knows**：廊下の撤収ボード「搬入口のスピーカーだけ本線（下手側）が断線。上手側の予備回線に振り替えています。客席とコンコースは通常どおり」。
- **Inference**：音が来ている入力＝放送源。搬入口は4回線あるが、メモが指すのは「上手側の予備」。
- **Answer**：入力6 →「客席スピーカー」「コンコース」「搬入口 上手側 予備」
- **Feedback**：入力違い／断線回線を選んだ／3か所そろっていない、を日本語で出し分ける（正誤以外は漏らさない）。
- **World change**：コンコース・客席に録音アナウンスが流れる（字幕＋チャイム）。内容に「6番出口へ」。
- **Hints**：①メーターとランプが生きている入力を示す。行き先は卓の養生テープ。②搬入口の続きは楽屋前の廊下の撤収ボード。③下手側の本線が断線＝上手側の予備へ。

### P5 暗転明けフォーカス（FOH 照明卓）
- **Prereq**：P6（照明リグ（上手）へ送電）がないと「リグに電源が来ていない」。
...
### P6 仮設分電盤（バックヤード）
- **表記の方針**：ブレーカー名はすべて日本語（照明リグ（上手/下手/中央）／音響アンプ（上手/下手）／音響・照明卓（FOH）／搬入口シャッター／ケータリング（保温庫））。
- **Sees**：入力ケーブルが届いていない分電盤、8系統のブレーカー、スタッフの付箋「発電機 燃料わずか！ 同時3系統まで」、盤の銘板（TOUR DISTRO 60A / OVERLOAD PROTECTED）。
  ※「空配線に送電するな」という注意書きは盤に貼らない（現実の機材にそんな貼り紙は無く、クイズに見える）。ルールは撤収ボードの申し送りと、実際に落としてみた時の反応で伝える。
- **Knows**：撤収ボードの申し送り「【電源】撤収の済んだ系統のブレーカーは上げないでください。機材を外したまま送ると保護装置が落ちて、盤ごと停まります。― K」。
  積込チェック表（表題は「ツアー機材」、凡例は ☑積込済／☐未積込、脚注「館の常設設備（卓・シャッター等）は対象外」。テープ色で記載：青照明×6・黄照明×2・赤音響アンプ・青音響アンプ・保温庫＝☑、赤照明×6＝☐「明朝吊り下ろし後」）。ステージ前ケースの色＝上手/下手/中央。
- **Inference**：①まず電源ドラムで入力をつなぐ。②撤収ボードの申し送りから「送電してよいのは機材が残っている系統だけ」と分かる（＝“なぜ残っているものを選ぶのか”の答え）。③色→上手/下手/中央に変換して積込済を消すと、残る機材は照明リグ（上手）だけ。④表に載らない常設設備（FOH・搬入口シャッター）を足してちょうど3系統＝付箋の上限と一致。
- **Feedback**：4つ目を入れると全落ち。空配線を選んだ場合は「〈○○〉は機材が運び出されたあとの空の回路だった」と、落ちた理由を名指しで返す。
- **World change**：分電盤の入力ランプ点灯、搬入口シャッターが上がる、照明リグが応答。
- **Hints**：①付箋は3系統まで。上げてはいけない理由は撤収ボードの【電源】。②何が運び出されたかは積込チェック表。色の意味はステージ前のケース。③赤＝上手・青＝下手・黄＝中央。残るのは照明リグ（上手）＋常設のFOH・搬入口シャッターの3つ。
...
- **Sees**：舞台側から描かれた平面図（上手＝F が左、5段目が上）。ブロックごとの ● は「退場確認の報告1件」。
  5段目は A・B・C に ●● が2つずつ、D・E・F には1つも無い（客席側と舞台側が同じ3区画を二重に確認していた証拠）。
  四隅に客席扉のアイコン（番号は書かれていない）。コンコース側に出口1〜6。
- **Knows**：チケット（E5ブロック57列7番＋裏面の座席図。客席扉1〜4の番号が入っている）。
  退場確認（客席側＝印刷された記号）と退場確認（舞台側＝記号が手書きで振り直されている）。
  自分が通ってきた扉の番号（客席扉4）。放送が告げた出口番号（6番）。
- **装置の理屈**：閉館シーケンス中は館内の扉を全施錠している。残留者対応モードは「ブロック → そのブロックに面した客席扉 →
  コンコースの出口」という経路を指定させ、**その経路の扉と出口だけ**を解錠して区間の誘導灯を点ける。
  退場確認の報告が上がっているブロックは指定できない（＝● が無い3区画しか受け付けない）。
  これが「なぜ自分のブロックを指定すると出口が開くのか」の答えで、盤面に明記してある。
- **Inference**：①舞台側の図面は左右が反転している（印刷の扉4が左上にあるのに、手書きの記号はAが左から始まっている）。
  ②報告が0件なのは D5・E5・F5 で、自分の席はそのうちの E5。舞台側から見ると左から2番目。
  ③扉は「そのブロックに面したもの」。E5＝後方の上手側なので、四隅の扉のうち扉4。舞台側から見た図では左上。
- **Feedback**：報告済みブロック→「指定できません」／別の空きブロック→「そちらで誘導灯が点いた。自分が立っているのはそこじゃない」／
  扉違い→「そのブロックからは遠すぎます」／出口違い→「この経路の先につながっていません」。
- **Answer**：ブロック＝上段左から2番目（E5）／扉＝左上（＝扉4）／出口＝6
- **Hints**：①この図面は誰の視点？ 報告が一度も付いていないブロックは？ ②記録のないブロックは3つ。自分の席はチケット。
  ③舞台側から見ると上手（E・F）は左。扉4は座席図では上手後方の角＝舞台側の図では左上。出口は放送の番号。
...
- 建築：多目的展示場型の大アリーナ（平土間＋2層のスタンド＋高い鉄骨トラス天井）、グレーのコンクリート、黒い折り畳み椅子、銀テープ、海沿いの夜景が見えるコンコース。
- **小道具はイラストを描かない。** 生成した実写素材をマゼンタ背景から切り抜き（`tools/fetch_images.py`）、影・明るさ・彩度を背景写真に合わせてから合成する（`src/scenes/overlays.tsx` の `prop()`）。位置は各写真を見て実測で合わせる。
...
- 状態変化用の写真（例：シャッターが開いた `backyard_open`）は、生成クレジットが無くても作れるように
  **元の写真のピクセルから合成する**（`tools/make_variants.py`）。別の写真を切り貼りすると必ず「貼った四角」に見えるため、
  夜の暗がり・床の照り返し・接地影として描き起こし、質感は同じ写真のコンクリートを流用する。
- 差し替える範囲（開口部）は手で多角形を引かず、`tools/shutter_mask.py` が写真から1px単位で切り出す：
  ①シャッター特有の「横スジ＋低彩度」で芯を取る ②明らかに別の材質（木＝高彩度／ケース＝暗く平坦／
  コンクリート・金具＝明るく平坦）に当たるまで芯から外へ広げ、境界のにじみ画素まで飲み込む
  ③手前の機材ケースは天板の直線当てはめ（右・前）と輪郭の塗りつぶし（左の重ね置き）で必ず除外
  ④左右の戸当たりは垂直、床との接地線は直線当てはめでクランプ。
  `python tools/shutter_mask.py --debug` で抽出範囲を色付きで確認できる。

## 10. スマホ対応（横持ち前提）
- `index.html` の viewport で `user-scalable=no, maximum-scale=1`、CSS で `touch-action: manipulation` / `overscroll-behavior: none` /
  `-webkit-touch-callout: none` を指定し、ピンチ拡大・端の引っぱり・長押しメニューを止める。
- 縦持ちで開くと全画面の案内（`.rotate-gate`）が出る。横向きにすれば自動で消える（どうしても縦で遊びたい人向けに閉じるボタンあり）。
- 横持ちで画面が低いとき（`orientation: landscape` かつ `max-height: 620px`）：
  - 持ち物バーは下の帯をやめて画面に重ねる浮きバーになり（`--bar-h: 0`）、16:9の画面が高さいっぱいに広がる。
  - 画面比が 16:9 より横長のときは、左右の余白をその場面のぼかし（`.stage-bg`）で埋めて全画面に見せる。
    切り抜き（cover）にしないのは、ホットスポットが写真の%座標に紐づいていて、切ると位置がずれるため。
  - 拡大画面（closeup）は高さいっぱいに表示し、「戻る」は右上に重ねる。スマホ画面は右端フル高さのパネルになる。
- 「調べられる場所」が分かるよう、ホットスポットに小さな目印を出す（設定 → 調べられる場所に目印／既定オン）。
  移動先の矢印も常時うっすら表示。`reduceMotion` のときは点滅させない。
- 文字サイズは `cqw`（フレーム幅基準）で拡縮し、下限を引き上げて小さい画面でも読めるようにしている。
- E2E は desktop（1280×800）と mobile（Pixel 5 横持ち 851×393）の2プロジェクトで同じ通し playthrough を回す。
